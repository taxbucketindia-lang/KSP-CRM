...
const EmployeeMaster = () => {
  ...
  const handleEdit = (emp) => {
    ...
    setFormData({
      ...
      password: '', 
      role: emp.userId?.role || 'Sales/Executive', 
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    try {
      const targetUserId = resetPassModal.employee.userId?._id || resetPassModal.employee.userId;
      if (!targetUserId) return toast.error("No portal login found for this employee.");

      await axios.put(`${import.meta.env.VITE_API_URL}/users/${targetUserId}/reset-password`, 
        { newPassword }, 
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 font-medium">Manage HR details and portal login access for your team.</p>
        </div>
                ...
                filteredEmployees.map((emp) => {
                  ...
                  <tr key={emp._id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-5">
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5">{emp.department || 'N/A'}</p>
                    </td>
                    <td className="py-3 px-5">
                    ...
                    <td className="py-3 px-5">
                      <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${
                        ...
                        emp.status === 'Resigned' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-slate-100 text-slate-600 border-slate-200'
                      }`}>
                        {emp.status}
                      </span>
                    </td>
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <form onSubmit={handleSave} className="overflow-y-auto p-6 space-y-6 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-3 bg-blue-50/40 p-5 rounded-2xl border border-blue-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} disabled={editingId} className="w-full p-2.5 border border-slate-200 rounded-xl text-sm font-semibold bg-white disabled:bg-slate-100 disabled:text-slate-500"/>
                    </div>
                    {!editingId && (
                      <div>
                    ...
                    <div className={editingId ? "md:col-span-2" : ""}>
                      ...
                      <select required disabled={editingId} value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})} className="w-full p-2.5 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 bg-white disabled:bg-slate-100 disabled:text-slate-500">
                ...
                <div className="md:col-span-3 mt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold uppercase text-slate-500 mb-1">Remarks</label>
                      <input type="text" placeholder="Internal notes" value={formData.remarks} onChange={(e) => setFormData({...formData, remarks: e.target.value})} className="w-full p-2.5 border border-slate-200 rounded-xl text-sm font-semibold"/>
                    </div>
                  </div>
                </div>
