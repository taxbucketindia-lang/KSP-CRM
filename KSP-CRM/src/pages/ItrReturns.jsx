...
const ItrReturns = () => {
  ...
  const generateNewClientId = () => {
    if (itrClients.length === 0) return 'ITR-1001';
    
    // Sort clients to find the highest number
    const sortedClients = [...itrClients].sort((a, b) => {
      // Assuming clientId format is ITR-XXXX
    ...
    return `ITR-${itrClients.length + 1001}`; 
  };
  ...
  const filteredClients = useMemo(() => {
    const filtered = itrClients.filter((client) => {
      ...
      // 🔴 NAYA: Search ab Client ID ke zariye bhi karega
      const matchesSearch = 
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const formattedItrRecords = data.map(row => ({
          clientId: getVal(row, ['Client ID', 'clientId', 'ID']),
  ...
  const handleOpenAdd = () => {
    ...
    setImportClientId('');
    // NAYA: Form kholte hi Auto ID Assign hogi
                    ...
                    <tr key={client._id} className={`transition-colors group ${isSelected ? 'bg-indigo-50/30' : 'hover:bg-slate-50/70'}`}>
                      ...
                      <td className="py-4 px-6">
                        ...
                        <div className="text-[11px] font-semibold text-slate-500 flex items-center gap-1.5 mt-1">
                            <span className="text-[10px] font-black uppercase tracking-wider bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200">
                             {client.clientId || 'N/A'}
                           </span>
                          </div>
                      </td>
                      <td className="py-4 pr-6">
                        <div className="font-mono text-sm font-bold text-slate-700 px-2 py-0.5 inline-block uppercase">
                          {client.pan || 'N/A'}
                        </div>
                      </td>
            ...
            <div className="flex justify-between items-center px-8 py-5 border-b border-slate-100 bg-slate-50/80">
              <div>
                <h2 className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
                  <FileText className="text-blue-600" size={24}/> {editMode ? 'Edit ITR Return Details' : 'Add / Import ITR Return'}
                </h2>
              ...
              {!editMode && (
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 p-5 rounded-2xl shadow-inner">
                  ...
                  <select value={importClientId} onChange={handleImportSelect} className="w-full text-sm border border-blue-200 bg-white rounded-xl p-3.5 font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/20 cursor-pointer shadow-sm">
                    <option value="">-- Click here to select a client --</option>
                    {importList.map(c => (
                      <option key={c._id} value={c._id}>{c.assesseeName} (PAN: {c.pan || 'N/A'}) - {c.mobile}</option>
                    ))}
                  </select>
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <input type="text" name="pan" required value={formData.pan} onChange={handleChange} disabled={editMode && !isAdmin} className="w-full text-sm font-bold border border-slate-200 rounded-xl p-3 uppercase focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="email" name="email" value={formData.email} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="assessee@example.com" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Date of Birth (DOB)</label>
                    <input type="date" name="dob" value={formData.dob} onChange={handleChange} disabled={editMode && !isAdmin} className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20 text-slate-700" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">District</label>
                    <input type="text" name="district" value={formData.district} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="e.g. South Delhi" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                  <div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input type="text" name="pinCode" value={formData.pinCode} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="110001" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div>
                    ...
                    <input type="text" name="bankName" value={formData.bankName} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="e.g. HDFC Bank" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">Account Number</label>
                    <input type="text" name="accountNo" value={formData.accountNo} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="Account No" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20 font-mono" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">IFSC Code</label>
                    <input type="text" name="ifscCode" value={formData.ifscCode} onChange={handleChange} disabled={editMode && !isAdmin} placeholder="HDFC0001234" className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20 uppercase font-mono" />
                  </div>
                </div>
              </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                      <div>
                        ...
                        <select name="itrFiledUpToAY" value={formData.itrFiledUpToAY} onChange={handleChange} className="w-full text-sm font-medium border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-blue-500/20">
                      ...
                      <div>
                        ...
                        <input type="date" name="filingDate" value={formData.filingDate} onChange={handleChange} className="w-full text-sm font-medium border border-emerald-200 rounded-xl p-3 focus:ring-2 focus:ring-emerald-500/20" />
                      </div>
                      <div>
                        <label className="block text-[11px] font-bold uppercase tracking-wider text-amber-600 mb-1.5">Next Year Reminder</label>
                        <input type="date" name="nextReminderDate" value={formData.nextReminderDate} onChange={handleChange} className="w-full text-sm font-medium border border-amber-200 rounded-xl p-3 focus:ring-2 focus:ring-amber-500/20" />
                      </div>
                    </div>
                 </div>
              </div>
